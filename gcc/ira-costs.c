		     ...
		     enum rtx_code index_code, int scale)
{
  ...
  switch (code)
	...
	add_cost = (ira_memory_move_cost[Pmode/*regmode*/][rclass][1] * scale) / 2;
	...
	pp_costs = pp->cost;
	ira_init_register_move_cost_if_necessary (Pmode/*regmode*/);
	move_in_cost = ira_may_move_in_cost[Pmode/*regmode*/];
	for (k = cost_classes_ptr->num - 1; k >= 0; k--)
	  {
	    i = cost_classes[k];
