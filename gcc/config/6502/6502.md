...
(define_mode_attr cmp_operand
  [(QI "compareqi_src_operand") (HI "multibyte_cmp_operand")])

; HImode/SImode operations.

(define_mode_iterator HISI [HI SI])
(define_mode_iterator HISIDI [HI SI DI])
...
(define_insn "addhi3_noclobacc"
  [(set (match_operand:HI 0 "register_operand"          "=&Rs,Rs")
	(plus:HI (match_operand:HI 1 "register_operand"   "Rs,Rs")
		 (match_operand:HI 2 "nonmemory_operand"  "Rs,i")))
   (clobber (reg:CC_NZ NZ_REGNUM))
   (clobber (reg:CC_C CARRY_REGNUM))
   (clobber (reg:CC_V OVERFLOW_REGNUM))]
  ...
  [(parallel
    [(set (pc) (if_then_else
		 (match_operator 0 "m65x_comparison"
	           [(match_operand:QIHI 1 "<cmp_operand>" "")
		    (match_operand:QIHI 2 "<cmp_operand>" "")])
  ...
  ""
{
  ...
  if (can_create_pseudo_p () &&
      !register_operand (operands[force_reg_operand], <MODE>mode))
    operands[force_reg_operand] = force_reg (<MODE>mode,
  ...
  if (<MODE>mode == QImode)
      ...
      case GTU:
	{
	  rtx insn
	    = gen_rtx_SET (pc_rtx,
			   gen_rtx_IF_THEN_ELSE (VOIDmode,
			     gen_rtx_fmt_ee (GET_CODE (operands[0]), VOIDmode,
					     operands[1], operands[2]),
			     gen_rtx_LABEL_REF (Pmode, operands[3]),
			     pc_rtx));
	  rtx par
	    = gen_rtx_PARALLEL (VOIDmode, rtvec_alloc (4));
	  XVECEXP (par, 0, 0) = insn;
	  ...
	  rtx_insn *jmp_insn = emit_jump_insn (par);
	  /*rtx note = find_reg_note (insn, REG_BR_PROB, NULL_RTX);
	  gcc_assert (note);*/
	  //add_int_reg_note (jmp_insn, REG_BR_PROB, REG_BR_PROB_BASE / 2);
	  DONE;
	}
	break;
      default:
...
(define_insn_and_split "cbrhi_eqne"
  [(set (pc) (if_then_else
	       (match_operator 0 "eqne_comparison"
	         [(match_operand:HI 1 "multibyte_cmp_operand" "SUcUjri")
		  (match_operand:HI 2 "multibyte_cmp_operand" "SUcUjri")])
	       (label_ref (match_operand 3 "" ""))
	       (pc)))
   (clobber (match_scratch:QI 4 "=hq"))
   (clobber (reg:CC_NZ NZ_REGNUM))
   (clobber (reg:CC_C CARRY_REGNUM))
   (clobber (reg:CC_V OVERFLOW_REGNUM))]
  ""
  ...
  [(const_int 0)]
{
  m65x_emit_himode_comparison (GET_CODE (operands[0]), operands[1],
			       operands[2], operands[3],
			       operands[4], BLOCK_FOR_INSN (curr_insn));
  DONE;
})

(define_insn_and_split "cbrhi_ltugeu"
  [(set (pc) (if_then_else
	       (match_operator 0 "ltugeu_comparison"
	         [(match_operand:HI 1 "multibyte_cmp_operand" "SUcUjri")
		  (match_operand:HI 2 "multibyte_cmp_operand" "SUcUjri")])
	       (label_ref (match_operand 3 "" ""))
	       (pc)))
...
(define_insn_and_split "cbrhi_leugtu"
  [(set (pc) (if_then_else
	       (match_operator 0 "leugtu_comparison"
		 [(match_operand:HI 1 "multibyte_cmp_operand" "SUcUjri")
		  (match_operand:HI 2 "multibyte_cmp_operand" "SUcUjri")])
	       (label_ref (match_operand 3 "" ""))
	       (pc)))
  ...
  [(const_int 0)]
{
  m65x_emit_himode_comparison (swap_condition (GET_CODE (operands[0])),
			       operands[2], operands[1], operands[3],
			       operands[4], BLOCK_FOR_INSN (curr_insn));
  DONE;
})

(define_insn_and_split "cbrhi_ltge"
  [(set (pc) (if_then_else
	       (match_operator 0 "ltge_comparison"
		 [(match_operand:HI 1 "multibyte_cmp_operand" "SUcUjri")
		  (match_operand:HI 2 "multibyte_cmp_operand" "SUcUjri")])
	       (label_ref (match_operand 3 "" ""))
	       (pc)))
...
(define_insn_and_split "cbrhi_legt"
  [(set (pc) (if_then_else
	       (match_operator 0 "legt_comparison"
		 [(match_operand:HI 1 "multibyte_cmp_operand" "SUcUjri")
		  (match_operand:HI 2 "multibyte_cmp_operand" "SUcUjri")])
	       (label_ref (match_operand 3 "" ""))
	       (pc)))
...
(define_expand "extendhisi2"
  [(set (match_operand:SI 0 "ptr_reg_operand" "")
	(sign_extend:SI (match_operand:HI 1 "ptr_reg_operand" "")))]
  "m65x_real_insns_ok ()"
{
...
(define_insn "movqi_noclob"
  [(set (match_operand:QI 0 "nonimmediate_operand"
	   "=hq,hq,Rs,hq,Uc,hq, <,hq, <,Aq,Aq, m,Rs,Uc,?hq, ?m,?Rs, ?m,Rshq")
	(match_operand:QI 1 "general_operand"
	     "i,Rs,hq,Uc,hq,hq,hq, >,Aq, >, m,Aq, z, z, ?m,?hq, ?m,?Rs,   0"))]
  "m65x_real_insns_ok ()
   && m65x_valid_mov_operands (QImode, operands, true)"
{
  ...
  [(set (attr "length")
	(symbol_ref "m65x_mov_length (insn, true)"))
   ...
   && m65x_valid_mov_operands (QImode, operands, true)"
{
  ...
  [(set (attr "length") (symbol_ref "m65x_mov_length (insn, false)"))
   (set_attr "arch" "*,*,*,*,*,*,phx,phx,no_phx,no_phx,*,*,cmos,cmos,*,*,\
		     *,*,*")])
...
(define_peephole2
  [(set (match_operand:QI 0 "accumulator_operand" "")
	(mem:QI (match_operand:HI 1 "zp_reg_operand" "")))]
  "!TARGET_ZPIND && m65x_real_insns_ok ()"
  [(pc)]
{
  if (m65x_peep_load (operands[0], operands[1], NULL_RTX, false))
    DONE;
  ...
  [(set (match_operand:QI 0 "accumulator_operand" "")
	(mem:QI
	  (plus:HI (zero_extend:HI (match_operand:QI 1 "y_reg_operand" ""))
		   (match_operand:HI 2 "ptr_reg_operand" ""))))]
  ""
  {})
...
(define_insn "loadqi_xind"
  [(set (match_operand:QI 0 "accumulator_operand" "=Aq")
	(mem:QI (unspec:QI [(match_operand:HI 1 "ptr_reg_operand" "Rs")
...
(define_expand "storeqi_indy"
  [(set (mem:QI
	  (plus:HI (zero_extend:HI (match_operand:QI 0 "y_reg_operand" ""))
		   (match_operand:HI 1 "ptr_reg_operand" "")))
	(match_operand:QI 2 "accumulator_operand" ""))]
  ""
  {})

(define_insn "storeqi_xind"
  [(set (mem:QI (unspec:QI [(match_operand:HI 0 "ptr_reg_operand" "Rs")
...
(define_insn "zp_clob_hardreg_movqi"
  [(set (match_operand:QI 0 "zp_reg_or_const_mem_operand" "=Rs,Uc,Rs,Uc")
	(match_operand:QI 1 "zp_reg_or_const_mem_operand"  "Rs,Rs,Uc,Uc"))
...
(define_insn "movqi_loadstore_zp"
  [(set (match_operand:QI 0 "reload_zpreg_operand" "=Rs")
	(match_operand:QI 1 "reload_zpreg_operand" "Rs"))
...
(define_subst "set_nz"
  [(set (match_operand:QI 0 "") (match_operand:QI 1 ""))
   (clobber (reg:CC_NZ NZ_REGNUM))]
  ""
  [(set (reg:CC_NZ NZ_REGNUM)
...
(define_subst_attr "nz" "set_nz" "" "_nz")
...
(define_insn "incdecqi3<nz>"
  [(set (match_operand:QI 0 "incdec_operand"
				"=Aq,Aq,xq,xq,yq,yq,Rs,Rs,UX,UX,Uc,Uc,ZX,ZX")
...
(define_insn "movcc_c"
  [(set (match_operand:CC_C 0 "register_operand"         "=cr")
	(match_operand:CC_C 1 "carry_zero_or_one_operand" "n"))]
  ""
{
  ...
  ""
{
  if (can_create_pseudo_p ()
      ...
      if (CONST_INT_P (operands[2]))
	{
	  ...
	  rtx tmp = gen_reg_rtx (QImode);
	  emit_move_insn (tmp, operands[1]);
	  emit_insn (gen_andqi3_insn (tmp, tmp, operands[2]));
	  emit_move_insn (operands[0], tmp);
	  DONE;
	}
    }
  ...
  ""
{
  ...
  switch (INTVAL (operands[2]))
    ...
    case 6:
      tmp = gen_reg_rtx (QImode);
      emit_insn (gen_ashlqi3_insn (tmp, operands[1], const1_rtx));
...
; These are actually the same, at least for the time being... do we need to
; define both?
(define_mode_iterator SFDF [SF DF])

(define_expand "copysign<mode>3"
  [(match_operand:SFDF 0 "nonimmediate_operand" "")
   (match_operand:SFDF 1 "general_operand" "")
   (match_operand:SFDF 2 "general_operand" "")]
  ""
{
  rtx op1 = gen_lowpart (SImode, operands[1]);
  rtx op2 = gen_lowpart (SImode, operands[2]);
  rtx tmp1 = expand_binop (SImode, and_optab, op1,
                           gen_int_mode (0xff7fffff, SImode), NULL_RTX, 1,
                           OPTAB_WIDEN);
  rtx tmp2 = expand_binop (SImode, and_optab, op2,
                           gen_int_mode (0x00800000, SImode), NULL_RTX, 1,
                           OPTAB_WIDEN);
  rtx tmp3 = expand_binop (SImode, ior_optab, tmp1, tmp2, NULL_RTX, 1,
                           OPTAB_WIDEN);
...
(define_expand "signbit<mode>2"
  ...
  ""
{
  rtx tmp = force_reg (QImode, const0_rtx);
  emit_move_insn (gen_highpart_mode (QImode, HImode, operands[0]), tmp);
  rtx part = simplify_gen_subreg (QImode, operands[1], <MODE>mode, 2);
  emit_insn (gen_compareqi (force_reg (QImode, part),
			    gen_int_mode (0x80, QImode)));
  emit_insn (gen_m65x_rolqi3 (tmp, tmp));
  emit_move_insn (gen_lowpart (QImode, operands[0]), tmp);
  DONE;
})
...
(define_expand "cstoreqi4"
  [(set (match_operand:QI 0 "accumulator_operand"    "=Aq,Aq,Aq,Aq,Aq,Aq,Aq")
	(match_operator:QI 1 "geu_comparison"
	  [(match_operand:QI 2 "hard_reg_operand"     "hq,hq,hq,Aq,Aq,Aq,Aq")
	   (match_operand:QI 3 "compareqi_src_operand" "r, i,Uc,Uy,Ur,ZX,Uj")]))
   (clobber (reg:CC_NZ NZ_REGNUM))
   (clobber (reg:CC_C CARRY_REGNUM))
   (clobber (reg:CC_V OVERFLOW_REGNUM))]
  ""
{
  /* There might be nice formulations for some other comparisons too.  */
  switch (GET_CODE (operands[1]))
    {
    case GEU:
      {
	rtx tmp = gen_reg_rtx (QImode);
	emit_insn (gen_compareqi (operands[2], operands[3]));
	emit_clobber (tmp);
	emit_insn (gen_m65x_rolqi3 (tmp, tmp));
  ...
  ""
{
  ...
  [(set (attr "length")
	(if_then_else (and (ge (minus (match_dup 1) (pc)) (const_int -126))
			   (lt (minus (match_dup 1) (pc)) (const_int 127)))
		      (const_int 2)
		      (const_int 5)))])
...
(define_insn "indirect_jump"
  [(set (pc)
	(match_operand:HI 0 "ptr_reg_operand" "r"))]
  ""
  "jmp (%0)"
  [(set_attr "length" "3")])
...
(define_insn_and_split "cbrqi_eqne"
  [(set (pc) (if_then_else
	       (match_operator 0 "eqne_comparison"
		 [(match_operand:QI 1 "hard_reg_operand"
						      "hq,hq,hq,Aq,Aq,Aq,Aq")
		  (match_operand:QI 2 "compareqi_src_operand"
						       "r, i,Uc,Uy,Ur,ZX,Uj")])
	       (label_ref (match_operand 3 "" ""))
	       (pc)))
   (clobber (reg:CC_NZ NZ_REGNUM))
  ...
  [(const_int 0)]
{
  m65x_emit_qimode_comparison (GET_CODE (operands[0]), operands[1], operands[2],
			       operands[3], NULL_RTX);
  DONE;
}
  [(set_attr "arch" "*,*,*,*,cmos,*,*")])

(define_insn_and_split "cbrqi_ltugeu"
  [(set (pc) (if_then_else
	       (match_operator 0 "ltugeu_comparison"
		 [(match_operand:QI 1 "hard_reg_operand"
						      "hq,hq,hq,Aq,Aq,Aq,Aq")
...
(define_insn_and_split "cbrqi_leugtu"
  [(set (pc) (if_then_else
	       (match_operator 0 "leugtu_comparison"
		 [(match_operand:QI 1 "compareqi_src_operand"
						       "r, i,Uc,Uy,Ur,ZX,Uj")
  ...
  [(const_int 0)]
{
  m65x_emit_qimode_comparison (swap_condition (GET_CODE (operands[0])),
			       operands[2], operands[1], operands[3],
			       NULL_RTX);
  DONE;
}
...
(define_insn_and_split "cbrqi_ltge"
  [(set (pc) (if_then_else
	       (match_operator 0 "ltge_comparison"
		 [(match_operand:QI 1 "hard_reg_operand"
						      "hq,hq,hq,Aq,Aq,Aq,Aq")
		  (match_operand:QI 2 "compareqi_src_operand"
	       ...
	       (label_ref (match_operand 3 "" ""))
	       (pc)))
   (clobber (match_scratch:QI 4 "=Aq,Aq,Aq,Aq,Aq,Aq,Aq"))
   (clobber (reg:CC_NZ NZ_REGNUM))
   (clobber (reg:CC_C CARRY_REGNUM))
   (clobber (reg:CC_V OVERFLOW_REGNUM))]
  ""
...
(define_insn_and_split "cbrqi_legt"
  [(set (pc) (if_then_else
	       (match_operator 0 "legt_comparison"
		 [(match_operand:QI 1 "compareqi_src_operand"
						       "r, i,Uc,Uy,Ur,ZX,Uj")
...
(define_insn "call_indirect_sym"
  [(set (match_operand 0 "" "")
	(call (mem:QI (match_operand:HI 1 "ptr_reg_operand" "r"))
	      (match_operand 2 "general_operand" "")))
   (use (const_int 0))]
  ""
{
...
; We don't actually care much if this works (it's only used for builtin_apply,
; which isn't very interesting for us), it's just to make test results
; better.
...
(define_insn "m65x_tsx"
  [(set (match_operand:QI 0 "x_reg_operand" "=xq")
        (unspec_volatile:QI [(match_operand:HI 1 "hard_sp_reg_operand" "hs")]
                            UNSPECV_TSX))
   (clobber (reg:CC_NZ NZ_REGNUM))]
  ""
  "tsx"
...
(define_insn "m65x_tsx_qi"
  [(set (match_operand:QI 0 "x_reg_operand" "=xq")
        (unspec_volatile:QI [(match_operand:QI 1 "hard_sp_reg_operand" "hs")]
                            UNSPECV_TSX))
   (clobber (reg:CC_NZ NZ_REGNUM))]
  ""
  "tsx"
...
(define_insn "m65x_txs"
  [(set (match_operand:HI 0 "hard_sp_reg_operand" "=hs")
        (unspec_volatile:HI [(match_operand:QI 1 "x_reg_operand" "xq")]
                            UNSPECV_TXS))
   (clobber (reg:CC_NZ NZ_REGNUM))]
  ""
  "txs"
...
(define_expand "save_stack_nonlocal"
  ...
  ""
{
  rtx soft_sp_slot = adjust_address (operands[0], HImode, 0);
  rtx hard_sp_slot = adjust_address (operands[0], QImode, 2);
  rtx hard_sp_reg = gen_rtx_REG (HImode, HARDSP_REGNUM);
  rtx xreg = gen_rtx_REG (QImode, X_REGNUM);
  emit_move_insn (soft_sp_slot, operands[1]);
  emit_insn (gen_m65x_tsx (xreg, hard_sp_reg));
  emit_move_insn (hard_sp_slot, xreg);
  DONE;
})
...
(define_expand "restore_stack_nonlocal"
  ...
  ""
{
  rtx soft_sp_slot = adjust_address (operands[1], HImode, 0);
  rtx hard_sp_slot = adjust_address (operands[1], QImode, 2);
  rtx hard_sp_reg = gen_rtx_REG (HImode, HARDSP_REGNUM);
  rtx xreg = gen_rtx_REG (QImode, X_REGNUM);
  emit_move_insn (xreg, hard_sp_slot);
  emit_insn (gen_m65x_txs (hard_sp_reg, xreg));
  emit_move_insn (operands[0], soft_sp_slot);
  DONE;
})
