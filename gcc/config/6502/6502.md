...
; For use in function prologue: copy a source register to a destination
; register and add an amount to the source register at the same time.  Used
; early in the prologue, when it's safe to clobber the accumulator
; unconditionally.
; Because of weirdness with RTX_FRAME_RELATED_P, this instruction should not
; use a splitter.

(define_insn "copy_addhi3_clobacc"
  [(set (match_operand:HI 0 "zp_reg_operand" "=r,r")
	(match_operand:HI 1 "zp_reg_operand"  "2,2"))
   (set (match_operand:HI 2 "zp_reg_operand" "=r,r")
	(plus:HI (match_operand:HI 3 "zp_reg_operand" "2,2")
		 (match_operand:HI 4 "immediate_operand" "z,i")))
   (clobber (reg:QI ACC_REGNUM))]
  ""
  "@
  lda %1\;sta %0\;lda %h1\;sta %h0
  lda %1\;sta %0\;clc\;adc #<%4\;sta %2\;lda %h1\;sta %h0\;adc #>%4\;sta %h2"
  [(set_attr "length" "8,17")])

(define_insn "addhi3_clobacc"
  [(set (match_operand:HI 0 "zp_reg_operand" "=r,r")
	(plus:HI (match_operand:HI 1 "zp_reg_operand" "r,r")
  ...
  [(pc)]
{
  ...
  emit_insn (gen_rtx_SET (VOIDmode, yreg, const0_rtx));
  emit_insn (gen_loadqi_indy (acc, yreg, operands[1]));
  emit_insn (gen_rtx_SET (VOIDmode, operands[0], acc));
