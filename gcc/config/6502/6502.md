  ...
  "can_create_pseudo_p ()"
{
  ...
  switch (shiftamt)
	...
	emit_insn (gen_andqi3 (tmphi, tmphi,
			       gen_int_mode (shiftamt == 6 ? 0x3 : 0x7,
					     QImode)));
  ...
  "can_create_pseudo_p ()"
{
  ...
  switch (shiftamt)
	...
	/* ashl #5			ashl #6
	   ...
	   hhhhhhhh llllllll ->		hhhhhhhh llllllll ->
	   llllllll hhhhhhhh ->		llllllll hhhhhhhh ->
	   hhhlllll lll00000		hhllllll ll000000.  */
	rtx tmp = gen_reg_rtx (QImode);
	rtx tmp2 = gen_reg_rtx (HImode), tmplo, tmphi;
	int i;

	emit_insn (gen_m65x_rorqi3 (tmp, gen_highpart_mode (QImode, HImode,
							    operands[1])));
	...
	emit_insn (gen_andqi3 (tmplo, tmplo,
			       gen_int_mode (shiftamt == 6 ? 0xc0 : 0xe0,
					     QImode)));
...
;(define_subst_attr "movqi_suf" "movqi" "" "_noclob")
;(define_subst_attr "movqi_saveflags" "movqi" "false" "true")
  ...
  [(pc)]
{
  ...
  if (save_needed)
    m65x_emit_movqi_nz (zpreg, acc);

  if (TARGET_ZPIND)
    {
      m65x_emit_movqi_nz (acc, gen_rtx_MEM (QImode, operands[1]));
  ...
  else
    {
      ...
      if (y_available)
	{
	  m65x_emit_movqi_nz (yreg, const0_rtx);
	  emit_insn (gen_loadqi_indy (acc, yreg, operands[1]));
	  m65x_emit_movqi_nz (operands[0], acc);
	}
      else if (x_available)
	{
	  m65x_emit_movqi_nz (xreg, const0_rtx);
  ...
  if (TARGET_ZPIND)
    {
      ...
      m65x_emit_movqi_nz (gen_rtx_MEM (QImode, operands[0]), acc);
    }
  else
    {
      bool x_available = peep2_regno_dead_p (1, X_REGNUM);
  ...
  [(pc)]
{
  ...
  m65x_emit_movqi_nz (acc, operands[2]);
  ...
  [(pc)]
{
  ...
  if (peep2_regno_dead_p (1, Y_REGNUM))
    {
      m65x_emit_movqi_nz (yreg, const0_rtx);
      emit_insn (gen_loadqi_indy (acc, yreg, operands[1]));
    }
  else if (peep2_regno_dead_p (1, X_REGNUM))
    {
      m65x_emit_movqi_nz (xreg, const0_rtx);
      emit_insn (gen_loadqi_xind (acc, operands[1], xreg));
    }
...
;(define_insn "load_sym_lo_noclob"
;  [(set (match_operand:QI 0 "hard_reg_operand" "=hq")
;	(truncate:QI (match_operand:HI 1 "sym_const_operand" "i")))]
;  ""
;  "php\;ld%R0 #<(%1)\;plp"
;  [(set_attr "length" "4")])

;(define_insn "load_sym_hi_noclob"
;  [(set (match_operand:QI 0 "hard_reg_operand" "=hq")
;	(truncate:QI
;	  (lshiftrt:HI (match_operand:HI 1 "sym_const_operand" "i")
;		       (const_int 8))))]
;  ""
;  "php\;ld%R0 #>(%1)\;plp"
;  [(set_attr "length" "4")])
...
(define_mode_attr ccmode_reg [(CC_C "40") (CC_UI "40") (CC_V "44")
			      (CC_NZ "36")])
