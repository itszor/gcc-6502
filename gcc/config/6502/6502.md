...
(define_insn_and_split "*movhi_insn"
  [(set (match_operand:HI 0 "mov_dst_operand"
				 "=hc,Ac,jc,hc, ?r,hc, U,Ac, m,r,r,r,U,U")
	(match_operand:HI 1 "mov_src_operand"
				   "i,jc,Ac, r,?hc, U,hc, m,Ac,r,i,U,r,z"))]
  ...
  [(pc)]
{
  if (MEM_P (operands[0])
	  ...
	  || m65x_indirect_indexed_addr_p (XEXP (operands[0], 0), true)))
    {
      ...
      DONE;
	   ...
	   && (REG_P (XEXP (operands[1], 0))
	       || m65x_indirect_indexed_addr_p (XEXP (operands[1], 0), true)))
    {
  ...
  ""
{
  switch (REGNO (operands[1]))
    {
    case ACC_REGNUM: return "sta %0\;ld%R2 _ah\;st%R2 %h0";
    case X_REGNUM: return "stx %0\;ld%R2 _xh\;st%R2 %h0";
    case Y_REGNUM: return "sty %0\;ld%R2 _yh\;st%R2 %h0";
    default: gcc_unreachable ();
    }
}
  [(set_attr "length" "6")])

(define_expand "reload_outhi_genregs"
...
(define_insn "storehi_genreg"
  [(set (match_operand:HI 0 "memory_operand" "=m")
	(match_operand:HI 1 "zp_reg_operand" "r"))
   (clobber (match_scratch:QI 2 "=Ah"))]
  ""
  "phy\;lda %1\;sta %0\;iny\;lda %h1\;sta %0\;ply"
  [(set_attr "length" "11")])

(define_expand "reload_inhi_genregs"
  [(parallel [(set (match_operand:HI 0 "zp_reg_operand" "=r")
		   (match_operand:HI 1 "memory_operand" "m"))
	      (clobber (match_operand:QI 2 "accumulator_operand" "=Ah"))])]
  ""
{})

(define_insn "loadhi_genreg"
  [(set (match_operand:HI 0 "zp_reg_operand" "=r")
	(match_operand:HI 1 "memory_operand" "m"))
   (clobber (match_scratch:QI 2 "=Ah"))]
  ""
  "phy\;lda %1\;sta %0\;iny\;lda %1\;sta %h0\;ply"
  [(set_attr "length" "11")])
...
;  
;  return "pha\;tya\;pha\;ldy #%1\;lda _yh\;sta (%0),y\;pla\;dey\;"
;	 "sta (%0),y\;tay\;pla";
;}
;  [(set_attr "length" "15")])
...
;  
;  return "pha\;ldy #%2\;lda (%1),y\;sta _yh\;dey\;lda (%1),y\;tay\;pla";
;}
;  [(set_attr "length" "12")])
  ...
  ""
{
  ...
  rtx indir_idx = gen_rtx_LO_SUM (HImode, XEXP (plus, 0),
		    gen_rtx_ZERO_EXTEND (HImode,
		      gen_rtx_REG (QImode, Y_REGNUM)));
  HOST_WIDE_INT offset = INTVAL (XEXP (plus, 1));
  rtx acc_lo = gen_lowpart (QImode, operands[0]);
  rtx acc_hi = gen_highpart (QImode, operands[0]);
  rtx mem;
  
  fprintf (stderr, "reload_inhi_acc_indy, op0=\n");
  debug_rtx (operands[0]);
  fprintf (stderr, "op1=\n");
  debug_rtx (operands[1]);
  
  mem = change_address (operands[1], QImode, indir_idx);

  emit_move_insn (operands[2], GEN_INT (offset + 1));
  emit_move_insn (acc_lo, mem);
  emit_move_insn (acc_hi, acc_lo);
  emit_move_insn (operands[2], GEN_INT (offset));
  emit_move_insn (acc_lo, mem);
...
(define_expand "reload_outhi_acc_indy"
  [(parallel [(set (match_operand:HI 0 "ind_y_mem_operand"  "=m")
	      ...
	      (clobber (match_operand:HI 2 "y_reg_operand" "=yh"))])]
  ""
{
  ...
  rtx indir_idx = gen_rtx_LO_SUM (HImode, XEXP (plus, 0),
		    gen_rtx_ZERO_EXTEND (HImode,
		      gen_rtx_REG (QImode, Y_REGNUM)));
  HOST_WIDE_INT offset = INTVAL (XEXP (plus, 1));
  rtx acc_lo = gen_lowpart (QImode, operands[1]);
  rtx acc_hi = gen_highpart (QImode, operands[1]);
  rtx y_lo = gen_lowpart (QImode, operands[2]);
  rtx y_hi = gen_highpart (QImode, operands[2]);
  rtx mem;

  fprintf (stderr, "reload_outhi_acc_indy, op0=\n");
  debug_rtx (operands[0]);
  fprintf (stderr, "op1=\n");
  debug_rtx (operands[1]);

  mem = change_address (operands[0], QImode, indir_idx);
  
  emit_move_insn (y_lo, GEN_INT (offset));
  emit_move_insn (mem, acc_lo);
  emit_move_insn (y_hi, acc_lo);
  emit_move_insn (y_lo, GEN_INT (offset + 1));
  emit_move_insn (acc_lo, acc_hi);
  emit_move_insn (mem, acc_lo);
  emit_move_insn (acc_lo, y_hi);
...
(define_insn "addhi3"
  [(set (match_operand:HI 0 "zp_reg_or_acc_operand"
	...
	(plus:HI (match_operand:HI 1 "zp_reg_or_acc_operand"
		 ...
		 (match_operand:HI 2 "zp_acc_imm_operand"
...
(define_insn "*movqi_insn"
  [(set (match_operand:QI 0 "mov_dst_operand"
		       "=hh,Ah,jh,hh, r,hh, U,Ah, m,r,U,r,xh,yh")
	(match_operand:QI 1 "mov_src_operand"
			 "i,jh,Ah, r,hh, U,hh, m,Ah,z,z,r,yh,xh"))]
  ...
  phy\;plx
  phx\;ply"
 [(set_attr "length" "2,1,1,2,2,3,3,2,2,2,3,6,2,2")])
...
;	(match_operand:QI 1 "mov_src_operand" ""))]
;  "reload_completed
;   && REG_P (operands[0]) && IS_ZP_REGNUM (REGNO (operands[0]))
;   && ((REG_P (operands[1]) && IS_ZP_REGNUM (REGNO (operands[1])))
;       || GET_CODE (operands[1]) == CONST_INT
;       || MEM_P (operands[1]))"
;  [(pc)]
;{
...
;(define_expand "reload_inqi_acc_indy"
...
;  [(set_attr "length" "4")])
;
;(define_expand "reload_outqi_acc_indy"
...
;  [(set_attr "length" "4")])
;
;; Store Y to indirect-Y location.
;
;(define_insn "movqi_sty_ind"
;  [(set (mem:QI (match_operand:HI 0 "reload_zpreg_operand" "r"))
...
;  [(set_attr "length" "5")])
;
;(define_insn "movqi_sty_indy"
;  [(set (mem:QI (plus:HI (match_operand:HI 0 "reload_zpreg_operand" "r")
;			 (match_operand:HI 1 "const_byte_amount"    "I")))
;	(match_operand:QI 2 "y_reg_operand" "yh"))]
;  ""
;  "pha\;tya\;ldy #%1\;sta (%0),y\;tay\;pla"
;  [(set_attr "length" "8")])
;
;; Load Y from indirect-Y location.
;
;(define_insn "movqi_ldy_ind"
...
;  [(set_attr "length" "5")])
;
;(define_insn "movqi_ldy_indy"
;  [(set (match_operand:QI 0 "y_reg_operand" "=yh")
;	(mem:QI (plus:HI (match_operand:HI 1 "reload_zpreg_operand" "r")
;			 (match_operand:HI 2 "const_byte_amount"    "I"))))]
;  ""
;  "pha\;ldy #%2\;lda (%1),y\;tay\;pla"
;  [(set_attr "length" "7")])
;
