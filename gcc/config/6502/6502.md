...
;	(plus:HI (high:HI (match_operand:HI 2 "y_reg_operand" "yh"))
;		 (match_operand:HI 1 "zp_reg_operand" "0")))
;   (clobber (match_scratch:QI 3 "=Aq"))]
;  ""
...
;		   (match_operand:HI 2 "y_reg_operand" "yh")))]
;  ""
...
(define_insn_and_split "*movsi_insn"
  [(set (match_operand:SI 0 "nonimmediate_operand"
			       "=hs,As,hs,xs,ys,hs, r,hs,Uc,As, m,r,r, r,Uc")
	(match_operand:SI 1 "general_operand"
				" i,hs,As,xs,ys, r,hs,Uc,hs, m,As,r,i,Uc, r"))]
  "m65x_valid_mov_operands (SImode, operands)"
  "#"
  "&& reload_completed"
  [(pc)]
{
  ...
  if (MEM_P (operands[0])
	  ...
	  || m65x_indirect_indexed_addr_p (SImode, XEXP (operands[0], 0),
					   true)))
    {
      ...
      if (REG_P (XEXP (operands[0], 0)))
        {
	  zpreg = XEXP (operands[0], 0);
	  offset = 0;
	}
      else if (GET_CODE (XEXP (operands[0], 0)) == PLUS
	       && CONST_INT_P (XEXP (XEXP (operands[0], 0), 1)))
	{
	  zpreg = XEXP (XEXP (operands[0], 0), 0);
	  offset = INTVAL (XEXP (XEXP (operands[0], 0), 1));
	}
      else
	gcc_unreachable ();
      
      emit_insn (gen_rtx_SET (VOIDmode, tmp1, acc));
      emit_insn (gen_rtx_SET (VOIDmode, tmp0, yreg));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, GEN_INT (offset)));
      emit_insn (gen_storeqi_indy (yreg, zpreg, acc));

      for (i = 1; i < 4; i++)
        {
	  emit_insn (gen_addqi3 (yreg, yreg, const1_rtx));
	  emit_insn (gen_rtx_SET (VOIDmode, acc,
				  gen_rtx_REG (QImode, ACC_REGNUM + i)));
	  emit_insn (gen_storeqi_indy (yreg, zpreg, acc));
	}

      emit_insn (gen_rtx_SET (VOIDmode, yreg, tmp0));
      emit_insn (gen_rtx_SET (VOIDmode, acc, tmp1));
      
      DONE;
    }
  else if (MEM_P (operands[1])
	   && (REG_P (XEXP (operands[1], 0))
	       || m65x_indirect_indexed_addr_p (SImode, XEXP (operands[1], 0),
						true)))
    {
      HOST_WIDE_INT offset;
      rtx zpreg;
	...
	{
	  zpreg = XEXP (operands[1], 0);
	  offset = 0;
	}
      else if (GET_CODE (XEXP (operands[1], 0)) == PLUS
	       && CONST_INT_P (XEXP (XEXP (operands[1], 0), 1)))
	{
	  zpreg = XEXP (XEXP (operands[1], 0), 0);
	  offset = INTVAL (XEXP (XEXP (operands[1], 0), 1));
	}
      else
	gcc_unreachable ();
      
      emit_insn (gen_rtx_SET (VOIDmode, tmp0, yreg));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, GEN_INT (offset + 3)));

      for (i = 3; i >= 1; i--)
        {
	  emit_insn (gen_loadqi_indy (acc, yreg, zpreg));
	  emit_insn (gen_rtx_SET (VOIDmode,
				  gen_rtx_REG (QImode, ACC_REGNUM + i), acc));
	  emit_insn (gen_addqi3 (yreg, yreg, constm1_rtx));
	}
  ...
  else if (immediate_operand (operands[1], SImode))
    {
      rtx op0_part, op1_part;
      
      if (REG_P (operands[0]) && IS_HARD_REGNUM (REGNO (operands[0])))
	{
	  ...
	  for (i = 3; i >= 1; i--)
	    {
	      emit_move_insn (op0_lo,
			      simplify_gen_subreg (QImode, operands[1],
						   SImode, i));
	      emit_move_insn (simplify_gen_subreg (QImode, operands[0],
						   SImode, i),
			      op0_lo);
	    }

	  emit_move_insn (op0_lo, gen_lowpart (QImode, operands[1]));
      ...
      if (REG_P (operands[0]) && IS_HARD_REGNUM (REGNO (operands[0])))
	{
	  ...
	  for (i = 3; i >= 1; i--)
	    {
	      emit_move_insn (op0_lo, simplify_gen_subreg (QImode, operands[1],
							   SImode, i));
	      emit_move_insn (simplify_gen_subreg (QImode, operands[0],
						   SImode, i),
			      op0_lo);
  ...
  emit_insn (gen_rtx_SET (VOIDmode, tmp0, acc));
  ...
  for (i = 0; i < 4; i++)
    {
      emit_insn (gen_rtx_SET (VOIDmode, acc,
			      simplify_gen_subreg (QImode, operands[1],
						   SImode, i)));
      emit_insn (gen_rtx_SET (VOIDmode,
			      simplify_gen_subreg (QImode, operands[0],
						   SImode, i), acc));
    }

  emit_insn (gen_rtx_SET (VOIDmode, acc, tmp0));
...
{
#if 0
  fprintf (stderr, "movhi, can_create_pseudo_p () = %s\n", can_create_pseudo_p () ? "yes" : "no");
  fprintf (stderr, "op0=\n");
	...
	operands[1] = copy_to_mode_reg (HImode, operands[1]);
	  ...
	  if (m65x_indirect_indexed_addr_p (HImode, XEXP (operands[0], 0),
					       false))
	    {
	      ...
	      if (!accumulator_operand (operands[1], HImode))
	        operands[1] = copy_to_mode_reg (HImode, operands[1]);
	    ...
	    {
	      emit_insn (gen_storehi_const_mem (operands[0], operands[1]));
	      DONE;
	    }
	}
      else if (MEM_P (operands[1]))
	  ...
	  if (m65x_indirect_indexed_addr_p (HImode, XEXP (operands[1], 0),
					    false))
	    {
	      ...
	      HOST_WIDE_INT hi_off = INTVAL (XEXP (plus, 1)) + 1;
	      rtx yreg = gen_reg_rtx (QImode);
	      rtx load_to = operands[0];

	      if (!accumulator_operand (load_to, HImode))
	        load_to = gen_reg_rtx (HImode);

	      emit_insn (gen_loadhi_highpart (load_to, XEXP (plus, 0),
					      GEN_INT (hi_off), yreg));
	      emit_insn (gen_loadhi_lowpart (load_to, XEXP (plus, 0),
					     XEXP (plus, 1), load_to, yreg));
	      if (operands[0] != load_to)
		emit_move_insn (operands[0], load_to);
	  ...
	  else if (zp_reg_operand (XEXP (operands[1], 0), HImode))
	    {
	      ...
	      if (REG_P (XEXP (mem_qimode, 0)))
	        {
		  emit_insn (gen_loadhi_highpart (load_to, XEXP (mem_qimode, 0),
						  GEN_INT (1), yreg));
		  emit_insn (gen_loadhi_lowpart (load_to, XEXP (mem_qimode, 0),
						 GEN_INT (0), load_to, yreg));
		  if (operands[0] != load_to)
		    emit_move_insn (operands[0], load_to);
      ...
      else if (!lra_in_progress && zp_or_hard_reg_operand (operands[0], HImode)
	       && zp_or_hard_reg_operand (operands[1], HImode))
        {
	  ...
	  emit_insn (gen_movhi_scratch (operands[0], operands[1]));
	  DONE;
	}
...
(define_insn_and_split "*movhi_insn"
  [(set (match_operand:HI 0 "nonimmediate_operand"
		     "=hh,Ah,hh,xh,yh,hh, r,hh,Uc,Ah, m,r,r, r,Uc,Uc,?hh, ?m")
	(match_operand:HI 1 "general_operand"
		       "i,hh,Ah,xh,yh, r,hh,Uc,hh, m,Ah,r,i,Uc, r, z, ?m,?hh"))]
  ...
  [(pc)]
{
  ...
  rtx yreg = gen_rtx_REG (QImode, Y_REGNUM);
  rtx acc = gen_rtx_REG (QImode, ACC_REGNUM);
  rtx acch = gen_rtx_REG (QImode, ACC_REGNUM + 1);
  ...
  if (MEM_P (operands[0])
	...
	gcc_unreachable ();
      ...
      emit_insn (gen_storeqi_indy (yreg, zpreg, acc));
      emit_insn (gen_addqi3 (yreg, yreg, const1_rtx));
      emit_insn (gen_rtx_SET (VOIDmode, acc, acch));
      emit_insn (gen_storeqi_indy (yreg, zpreg, acc));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, tmp0));
      emit_insn (gen_rtx_SET (VOIDmode, acc, tmp1));
    }
	...
	gcc_unreachable ();
      ...
      emit_insn (gen_rtx_SET (VOIDmode, tmp0, yreg));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, GEN_INT (offset + 1)));
      emit_insn (gen_loadqi_indy (acc, yreg, zpreg));
      emit_insn (gen_rtx_SET (VOIDmode, acch, acc));
      emit_insn (gen_addqi3 (yreg, yreg, constm1_rtx));
      emit_insn (gen_loadqi_indy (acc, yreg, zpreg));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, tmp0));
    }
    ...
    {
      rtx imm_lo, imm_hi;
      rtx reg_lopart = gen_lowpart (QImode, operands[0]);
      rtx reg_hipart = gen_highpart_mode (QImode, HImode, operands[0]);

      if (CONST_INT_P (operands[1]))
	{
	  imm_lo = gen_lowpart (QImode, operands[1]);
	  imm_hi = gen_highpart_mode (QImode, HImode, operands[1]);
	}
      else
        {
	  imm_lo = gen_rtx_UNSPEC (QImode, gen_rtvec (1, operands[1]),
				   UNSPEC_LOADSYM_LO);
	  imm_hi = gen_rtx_UNSPEC (QImode, gen_rtvec (1, operands[1]),
				   UNSPEC_LOADSYM_HI);
	}

      if (REG_P (operands[0]))
        {
	  if (IS_HARD_REGNUM (REGNO (operands[0])))
	    {
	      emit_insn (gen_rtx_SET (VOIDmode, reg_lopart, imm_hi));
	      emit_insn (gen_rtx_SET (VOIDmode, reg_hipart, reg_lopart));
	      emit_insn (gen_rtx_SET (VOIDmode, reg_lopart, imm_lo));
	    }
	  else
	    {
	      emit_insn (gen_rtx_SET (VOIDmode, tmp0, acc));
	      emit_insn (gen_rtx_SET (VOIDmode, acc, imm_hi));
	      emit_insn (gen_rtx_SET (VOIDmode, reg_hipart, acc));
	      emit_insn (gen_rtx_SET (VOIDmode, acc, imm_lo));
	      emit_insn (gen_rtx_SET (VOIDmode, reg_lopart, acc));
	      emit_insn (gen_rtx_SET (VOIDmode, acc, tmp0));
	    }
	}
      else
...
(define_insn "safe_wordmove"
  [(set (match_operand:QI 0 "mov_dst_operand" "=r,r, r,Uc, r,Uc")
	(match_operand:QI 1 "mov_src_operand"  "r,i,hq,hq,Uc, r"))
   (set (match_operand:QI 2 "mov_dst_operand" "=r,r, r,Uc, r,Uc")
	(match_operand:QI 3 "mov_src_operand"  "r,i, r, r,Uc, r"))]
...
(define_peephole2
  [(match_scratch:QI 2 "hq")
   (set (match_operand:HI 0 "zp_reg_operand" "")
	(unspec:HI [(match_operand:HI 1 "immediate_operand" "")]
		   UNSPEC_LOADHI_IMMED_PR))]
  ""
...
(define_insn "storehi_lowpart"
  [(set (mem:QI (plus:HI (match_operand:HI 0 "zp_reg_operand"     "r")
			 (match_operand:HI 1 "const_byte_amount"  "I")))
	(truncate:QI (match_operand:HI 2 "accumulator_operand"     "Ah")))
   (set (match_operand:QI 3 "y_reg_operand" "=yq")
...
(define_insn "loadhi_lowpart"
  [(set (match_operand:HI 0 "accumulator_operand" "=Ah")
	(ior:HI
	  (zero_extend:HI
	    (mem:QI (plus:HI (match_operand:HI 1 "zp_reg_operand" "r")
			     (match_operand:HI 2 "const_byte_amount" "I"))))
	  (and:HI (match_operand:HI 3 "accumulator_operand" "0")
		  (const_int 65280))))
   (set (match_operand:QI 4 "y_reg_operand" "=yq")
...
(define_insn "loadhi_highpart"
  [(set (match_operand:HI 0 "accumulator_operand" "=Ah")
   ...
   (set (match_operand:QI 3 "y_reg_operand" "=yq")
	(truncate:QI (match_dup 2)))]
  ""
  "ldy #%2\;lda (%1),y\;sta _Aq"
  [(set_attr "length" "6")])

(define_insn "loadhi_constmem"
  [(set (match_operand:HI 0 "hard_reg_operand" "=hh")
	(unspec:HI [(match_operand:HI 1 "const_mem_operand" "Uc")]
		   UNSPEC_LOADHI_CONSTMEM))]
  ""
...
(define_insn "loadhi_immed"
  [(set (match_operand:HI 0 "register_operand" "=hh,r")
...
(define_insn "movhi_scratch"
  [(set (match_operand:HI 0 "reg_or_const_mem_operand"
			       "=Ah,Ah,Ah,xh,xh,yh,yh,hh, r, r,Ah,Uc, m,Ah")
	(match_operand:HI 1 "general_operand"
				"Ah,xh,yh,xh,Ah,Ah,yh, r,hh, r,Uy, z,Ah, m"))
   (clobber (match_scratch:QI 2 "=X, X, X, X, X, X, X, X,hq,hq,yq,hq,yq,yq"))]
  ""
  "@
  nop ; A->A
  lda _xh\;sta _ah\;txa
  ...
  #
  ld%R2 #0\;st%R2 %0\;st%R2 %0+1
  ldy #0\;sta (0),y
  ldy #0\;lda (0),y"
  [(set_attr "length" "2,5,5,2,5,5,2,6,6,8,*,8,4,4")])

(define_split
  [(set (match_operand:HI 0 "accumulator_operand" "=Ah")
	(mem:HI (plus:HI (match_operand:HI 1 "zp_reg_operand" "r")
			 (match_operand:HI 2 "const_byte_amount" "I"))))
   (clobber (match_scratch:QI 3 "=yq"))]
  ""
  [(parallel [(set (match_dup 0)
		   (mult:HI
		     (zero_extend:HI
		       (mem:QI (plus:HI (match_dup 1) (match_dup 2))))
		     (const_int 257)))
	      (set (match_dup 3) (truncate:QI (match_dup 2)))])
   (parallel [(set (match_dup 0)
		   (ior:HI
		     (zero_extend:HI
		       (mem:QI (plus:HI (match_dup 1) (match_dup 4))))
		     (and:HI (match_dup 0) (const_int 65280))))
	      (set (match_dup 3) (truncate:QI (match_dup 4)))])]
{
  operands[4] = GEN_INT (INTVAL (operands[2]) + 1);
})

(define_insn "storehi_const_mem"
  [(set (match_operand:HI 0 "const_mem_operand" "=Uc")
	(match_operand:HI 1 "register_operand"   "hh"))
...
(define_insn "*addhi3_insn"
  [(set (match_operand:HI 0 "register_operand"
					       "=r, r,Ah,Ah,Ah,Ah,r,r,r,r")
	(plus:HI (match_operand:HI 1 "register_operand"
					       "%r,Ah, 0, 0, 0, 0,r,0,0,r")
		 (match_operand:HI 2 "nonmemory_operand"
					       "Ah, i, r, K, I, i,r,K,I,i")))]
  ""
  "@
...
(define_insn "addhi3_zp_clob_acc"
  [(set (match_operand:HI 0 "zp_reg_operand"		     "=r, r,r,r,r,r")
	(plus:HI (match_operand:HI 1 "zp_reg_or_acc_operand" "%r,Ah,r,0,0,r")
		 (match_operand:HI 2 "nonmemory_operand"     "Ah, i,r,K,I,i")))
   (clobber (match_scratch:QI 3 "=Aq,Aq,Aq,X,Aq,Aq"))]
  ""
  "@
...
(define_insn "*lo_sum_addr"
  [(set (match_operand:HI 0 "zp_reg_operand" "=r")
	(plus:HI (zero_extend:HI
		   (match_operand:QI 2 "y_reg_operand" "yq"))
		 (match_operand:HI 1 "zp_reg_operand" "0")))]
  ""
...
(define_insn_and_split "cbrhi_eq"
  [(set (pc) (if_then_else
	       (eq (match_operand:HI 0 "register_operand" "hh,hh,hh,r,r")
		   (match_operand:HI 1 "mov_src_operand"   "r, i,Uc,r,i"))
	       (label_ref (match_operand 2 "" ""))
	       (pc)))
   (clobber (match_scratch:QI 3 "=hq,hq,hq,hq,hq"))]
  ""
  "#"
  "reload_completed"
  [(pc)]
...
(define_insn_and_split "cbrhi_ne"
  [(set (pc) (if_then_else
	       (ne (match_operand:HI 0 "register_operand" "hh,hh,hh,r,r")
		   (match_operand:HI 1 "mov_src_operand"   "r, i,Uc,r,i"))
	       (label_ref (match_operand 2 "" ""))
	       (pc)))
   (clobber (match_scratch:QI 3 "=hq,hq,hq,hq,hq"))]
...
(define_insn_and_split "cbrhi_ltu"
  [(set (pc) (if_then_else
	       (ltu (match_operand:HI 0 "register_operand" "hh,hh,r,hh,r")
		    (match_operand:HI 1 "mov_src_operand"   "r, i,i,Uc,r"))
	       (label_ref (match_operand 2 "" ""))
	       (pc)))
   (clobber (match_scratch:QI 3 "=&hq,&hq,hq,&hq,hq"))]
  ""
...
(define_insn_and_split "cbrhi_gtu"
  [(set (pc) (if_then_else
	       (gtu (match_operand:HI 0 "mov_src_operand"   "r, i,i,Uc,r")
		    (match_operand:HI 1 "register_operand" "hh,hh,r,hh,r"))
	       (label_ref (match_operand 2 "" ""))
	       (pc)))
   (clobber (match_scratch:QI 3 "=&hq,&hq,hq,&hq,hq"))]
  ""
...
(define_insn_and_split "cbrhi_geu"
  [(set (pc) (if_then_else
	       (geu (match_operand:HI 0 "register_operand"      "hh,hh,r,r")
		    (match_operand:HI 1 "zp_reg_or_imm_operand"  "r, i,r,i"))
	     (label_ref (match_operand 2 "" ""))
	     (pc)))
   (clobber (match_scratch:QI 3 "=&hq,&hq,hq,hq"))]
  ""
  "#"
  "reload_completed"
  [(pc)]
...
(define_insn_and_split "cbrhi_lt"
  [(set (pc) (if_then_else
	       (lt (match_operand:HI 0 "register_operand" "hh,hh,r,r")
		   (match_operand:HI 1 "mov_src_operand"   "r, i,r,i"))
	       (label_ref (match_operand 2 "" ""))
	       (pc)))
   (clobber (match_scratch:QI 3 "=Aq,Aq,Aq,Aq"))]
  ""
  "#"
  "reload_completed"
  [(pc)]
...
(define_insn_and_split "cbrhi_ge"
  [(set (pc) (if_then_else
	       (ge (match_operand:HI 0 "register_operand" "hh,hh,r,r")
		   (match_operand:HI 1 "mov_src_operand"   "r, i,r,i"))
	       (label_ref (match_operand 2 "" ""))
	       (pc)))
   (clobber (match_scratch:QI 3 "=Aq,Aq,Aq,Aq"))]
...
#if 0
  fprintf (stderr, "lra_in_progress: %d, reload_completed: %d, can_create_pseudo_p () = %d\n", lra_in_progress, reload_completed, can_create_pseudo_p ());
  fprintf (stderr, "expand movqi: op0=\n");
  debug_rtx (operands[0]);
  fprintf (stderr, "op1=\n");
  debug_rtx (operands[1]);
#endif
...
(define_insn "*movqi_insn"
  [(set (match_operand:QI 0 "mov_dst_operand"
       "=hq,Aq,hq,r,hq, r,hq,Uc,Aq, m,r,Uc,xq,yq,?hq, ?m, *r")
	(match_operand:QI 1 "mov_src_operand"
	 "i,hq,Aq,r, r,hq,Uc,hq, m,Aq,z, z,yq,xq, ?m,?hq,*Ur"))]
  ...
  #
  pha\;lda %1\;sta %0\;pla"
 [(set_attr "length" "2,1,1,*,2,2,3,3,*,*,2,3,2,2,*,*,7")])
  ...
  [(pc)]
{
  ...
  if (REGNO (operands[0]) == ACC_REGNUM)
    {
      emit_insn (gen_rtx_SET (VOIDmode, tmp0, yreg));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, operands[2]));
	...
	emit_insn (gen_rtx_SET (VOIDmode, tmp0, yreg));
      ...
      if (REGNO (operands[0]) != Y_REGNUM)
	emit_insn (gen_rtx_SET (VOIDmode, yreg, tmp0));
      emit_insn (gen_rtx_SET (VOIDmode, operands[0], acc));
      emit_insn (gen_rtx_SET (VOIDmode, acc, tmp1));
    }
...
(define_split
  ...
  ""
  [(set (mem:QI (plus:HI (match_dup 0) (match_dup 1)))
	(unspec:QI [(match_dup 2)] UNSPEC_STOREQI))])
...
(define_split
  ...
  [(set (mem:QI (plus:HI (match_dup 0) (const_int 0)))
	(unspec:QI [(match_dup 1)] UNSPEC_STOREQI))])
...
(define_insn_and_split "*storeqi_indy_tmps"
  [(set (mem:QI (plus:HI (match_operand:HI 0 "zp_reg_operand" "=r,?r")
	...
	(unspec:QI [(match_operand:QI 2 "hard_reg_operand" "=Aq,?jq")]
		   UNSPEC_STOREQI))]
  ""
  "#"
  "reload_completed"
  [(pc)]
{
  ...
  if (REGNO (operands[2]) == ACC_REGNUM)
    {
      emit_insn (gen_rtx_SET (VOIDmode, tmp0, yreg));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, operands[1]));
  ...
  else
    {
      ...
      emit_insn (gen_rtx_SET (VOIDmode, tmp0, yreg));
      emit_insn (gen_rtx_SET (VOIDmode, tmp1, acc));
      emit_insn (gen_rtx_SET (VOIDmode, acc, operands[2]));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, operands[1]));
      emit_insn (gen_storeqi_indy (yreg, operands[0], acc));
      emit_insn (gen_rtx_SET (VOIDmode, acc, tmp1));
      emit_insn (gen_rtx_SET (VOIDmode, yreg, tmp0));
...
(define_peephole2
  [(match_scratch:QI 3 "yq")
   (set (mem:QI (plus:HI (match_operand:HI 0 "zp_reg_operand" "")
			 (match_operand:HI 1 "const_byte_amount" "")))
	(unspec:QI [(match_operand:QI 2 "accumulator_operand" "")]
		   UNSPEC_STOREQI))]
  ""
...
(define_peephole2
  [(match_scratch:QI 3 "yq")
   ...
   (set (mem:QI (plus:HI (match_operand:HI 0 "zp_reg_operand" "")
			 (match_operand:HI 1 "const_byte_amount" "")))
	(unspec:QI [(match_operand:QI 2 "index_reg_operand" "")]
		   UNSPEC_STOREQI))]
  ""
...
(define_insn "load_sym_lo"
  [(set (match_operand:QI 0 "hard_reg_operand" "=hq")
	(unspec:QI [(match_operand:HI 1 "sym_const_operand" "i")]
		   UNSPEC_LOADSYM_LO))]
  ""
  "ld%R0 #<(%1)"
  [(set_attr "length" "2")])
...
(define_insn "pushqi1"
  [(set (reg:QI HARDSP_REGNUM) (plus:QI (reg:QI HARDSP_REGNUM) (const_int -1)))
   (set (mem:QI (reg:QI HARDSP_REGNUM))
	(match_operand:QI 0 "hard_reg_operand" "hq"))]
  ""
  "ph%R0"
  [(set_attr "length" "1")])
...
;(define_expand "reload_y_inqi"
;  [(parallel [(set (match_operand:QI 0 "y_reg_operand" "=yq")
;		   (match_operand:QI 1 "memory_operand" "Uy"))
;	      (clobber (match_operand:QI 2 "accumulator_operand" "=Aq"))])]
;  ""
;{})
;
;(define_insn "reload_y_inqi_indy"
;  [(set (match_operand:QI 0 "y_reg_operand" "=yq")
;	(mem:QI (plus:HI (match_operand:HI 1 "zp_reg_operand" "r")
;			 (match_operand:HI 2 "const_byte_amount" "i"))))
;   (clobber (match_scratch:QI 3 "=Aq"))]
;  ""
...
;  [(set_attr "length" "5")])
;
;(define_expand "reload_y_outqi"
;  [(parallel [(set (match_operand:QI 0 "memory_operand" "Uy")
;		   (match_operand:QI 1 "y_reg_operand" "=yq"))
;	      (clobber (match_operand:QI 2 "accumulator_operand" "=Aq"))])]
;  ""
;{})
;
;(define_insn "reload_y_outqi_indy"
;  [(set (mem:QI (plus:HI (match_operand:HI 0 "zp_reg_operand" "r")
;			 (match_operand:HI 1 "const_byte_amount" "i")))
;	(match_operand:QI 2 "y_reg_operand" "=yq"))
;   (clobber (match_scratch:QI 3 "=Aq"))]
;  ""
;  "tya\;ldy #%1\;sta (%0),y\;tay"
;  [(set_attr "length" "6")])
  ...
  ""
{
  switch (INTVAL (operands[2]))
    {
    case 1: return "asl a";
    case 2: return "asl a\;asl a";
    case 3: return "asl a\;asl a\;asl a";
    case 4: return "asl a\;asl a\;asl a\;asl a";
    case 5: return "asl a\;asl a\;asl a\;asl a\;asl a";
    case 6: return "ror a\;ror a\;ror a\;and #$c0";
    case 7: return "ror a\;ror a\;and #$80";
    case 8: return "lda #0";
    default: gcc_unreachable ();
    }
}
  [(set (attr "length")
	(cond [(eq (match_dup 2) (const_int 1)) (const_int 1)
	       (eq (match_dup 2) (const_int 2)) (const_int 2)
	       (eq (match_dup 2) (const_int 3)) (const_int 3)
	       (eq (match_dup 2) (const_int 4)) (const_int 4)
	       (eq (match_dup 2) (const_int 5)) (const_int 5)
	       (eq (match_dup 2) (const_int 6)) (const_int 5)
	       (eq (match_dup 2) (const_int 7)) (const_int 4)
	       (eq (match_dup 2) (const_int 8)) (const_int 2)]
	      (const_int 0)))])
  ...
  ""
{
  switch (INTVAL (operands[2]))
    {
    case 1: return "lsr a";
    case 2: return "lsr a\;lsr a";
    case 3: return "lsr a\;lsr a\;lsr a";
    case 4: return "lsr a\;lsr a\;lsr a\;lsr a";
    case 5: return "lsr a\;lsr a\;lsr a\;lsr a\;lsr a";
    case 6: return "rol a\;rol a\;rol a\;and #$3";
    case 7: return "rol a\;rol a\;and #1";
    case 8: return "lda #0";
    default: gcc_unreachable ();
    }
}
...
(define_insn "indirect_jump"
  [(set (pc)
	(match_operand:HI 0 "zp_reg_operand" "Ah,r"))]
  ""
  "@
